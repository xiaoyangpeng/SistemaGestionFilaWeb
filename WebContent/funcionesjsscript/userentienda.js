 ...
 */ //回调函数
		...
		window.setInterval("cargarPagina()",10000)
		 ...
		 function cargarPagina(){
		  ...
		  req.open("GET", "/proyectoFinalEntrada/clienteentienda", true);
		 ...
		 if(data.length!=0){
			      ...
			      miturno.innerHTML=data[0].turno_actual;
				    totalturno.innerHTML="Turno total : "+(data.length+1);
					 ...
					 miturno.innerHTML="No queda Nadie En La Tienda";
		    ...
		    function getDataRow(h){ 
	    	    ...
	    	    var terminar=document.createElement('td');//创建第四列，操作列  
	    	     
	    	     row.appendChild(terminar);  
	    	     
	    	     var bt2 =document.createElement("button");           //createElement生成button对象
			     bt2.innerHTML = 'Terminar'; 
			     bt2.onclick = function () {                          //绑定点击事件
			   				
			     	
							     	       
						  req.open("GET", "/proyectoFinalEntrada/clienteentienda?id_usuario="+h.id_usuario, true);
						        //如果设置数据传送方式为post，则必须设置请求头信息
	    	  ...
	    	  terminar.appendChild(bt2);  //把删除按钮加入td，别忘了   
